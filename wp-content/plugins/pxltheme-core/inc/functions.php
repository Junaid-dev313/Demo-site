...
if(!function_exists('pxl_post_share_link')){
    function pxl_post_share_link($args=[]){
        ...
        <?php switch ($args['icon']) {
            ...
            case 'pinterest':
                ?>
                <a class="<?php echo esc_attr($args['link_class'])?> icon-pinterest" target="_blank" href="http://pinterest.com/pin/create/button/?url=<?php echo urlencode($url); ?>&media=<?php echo esc_attr($img_url); ?>&description=<?php echo esc_attr($title); ?>">
                    <?php 
                    if( !empty( $args['icon_class'] )){
                        echo '<span class="'.$args['icon_class'].'"></span>';
                    }
