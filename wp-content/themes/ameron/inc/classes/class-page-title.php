<?php
if (!class_exists('Ameron_Page_Title')) {
    class Ameron_Page_Title
    {
        public function get_page_title(){
            $pt_mode = ameron()->get_opt('pt_mode');
            if( $pt_mode == 'none' ) return;

            $ptitle_layout = (int)ameron()->get_opt('ptitle_layout');
            $titles = $this->get_title();

            $breadcrumb_on = ameron()->get_opt( 'breadcrumb_on', false );
        ...
        public function get_title() {
            $title = '';
            $single_post_title_layout = ameron()->get_theme_opt('single_post_title_layout','0');
            $post_custom_title  = ameron()->get_theme_opt('post_custom_title',esc_html__('Blog details', 'ameron'));
            // Default titles
            if ( ! is_archive() ) {
                // Posts page view
                if ( is_home() ) {
                ...
                elseif ( is_page() ) {
                    ...
                    $title = esc_html__( '404', 'ameron' );
                } elseif ( is_search() ) {
                    $title = esc_html__( 'Search results', 'ameron' );
                } else {
                    $title = get_post_meta( get_the_ID(), 'custom_title', true );
                ...
                $custom_title = ameron()->get_opt('custom_title');
            ...
            $main_title = ameron()->get_page_opt('custom_main_title');
            if (!empty($main_title)){
                $title = $main_title;
            }
            $sub_title = ameron()->get_opt('custom_sub_title');
            $page_sub_title = ameron()->get_page_opt('custom_sub_title');
