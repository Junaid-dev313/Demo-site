<?php
if (!class_exists('Ameron_Comment')) {
     
    class Ameron_Comment
    {
        ...
        public function comment_extra_control ( $comment ) {
            ...
            <p>
                <label for="phone"><?php esc_html_e( 'Phone','ameron' ); ?></label>
            ...
            ?>
            <p>
                <label for="subject"><?php esc_html_e( 'Subject','ameron' ); ?></label>
            ...
            <p>
                <label for="rating"><?php esc_html_e( 'Rating: ','ameron' ); ?></label>
        ...
        public function comment_rating_fields ($args =[]) {
            ...
            if($this->show_rating == '1' && is_singular('post')){
                $rating .= '<div class="pxl-comment-form-rating pxl-comment-form-fields-wrap row gx-20 align-items-center '.esc_attr($args['class']).'">';
                    $rating .= '<div  class="comment-form-field col-auto">'. esc_html__('Your Rating','ameron').'<span class="required">*</span></div>';
        ...
        function wc_comment_rating_fields($args =[]){
            ...
            if (wc_review_ratings_enabled() && is_singular('product') ) {
                $rating .= '<div class="pxl-comment-form-rating pxl-comment-form-fields-wrap row gutters-12 align-items-center '.esc_attr($args['class']).'">';
                    $rating .= '<div class="comment-form-field col-auto">' . esc_html__( 'Your rating of this product', 'ameron' ) . ( wc_review_ratings_required() ? '&nbsp;<span class="required">*</span>' : '' ) . '</div>';
                    $rating .= '<div class="comment-form-field comments-rating col">';
                        $rating .= '<select name="rating" id="rating" required>
                            <option value="">' . esc_html__( 'Rate&hellip;', 'ameron' ) . '</option>
                            <option value="5">' . esc_html__( 'Perfect', 'ameron' ) . '</option>
                            <option value="4">' . esc_html__( 'Good', 'ameron' ) . '</option>
                            <option value="3">' . esc_html__( 'Average', 'ameron' ) . '</option>
                            <option value="2">' . esc_html__( 'Not that bad', 'ameron' ) . '</option>
                            <option value="1">' . esc_html__( 'Very poor', 'ameron' ) . '</option>
                        </select>';
                    $rating .= '</div>';
                $rating .= '</div>';
            }
        ...
        public function comment_rating_require_rating( $commentdata ) {
            ...
            wp_die( esc_html__( 'Error: You did not add a rating. Hit the Back button on your Web browser and resubmit your comment with a rating.','ameron' ) );
            return $commentdata;
        }
                        ...
                        <div class="inner-top row gx-20 align-items-center">
                            <?php if ($args['avatar_size'] != 0) : ?>
                                <div class="comment-avatar col-auto empty-none"><?php
                                    echo get_avatar($comment, '115');
                                    ?></div>
                            <?php endif; ?>
                            <div class="comment-content col">
                                <div class="row g-10 align-items-center justify-content-between">
                                    <div class="comment-box col-auto d-flex align-items-baseline">
                                ...
                                <div class="col-auto comment-reply-post">
                                    <?php
                                    comment_reply_link( array_merge( $args, array(
                                        'add_below' => $add_below,
                                        'depth'     => $depth,
                                        'max_depth' => $args['max_depth'],
                                        'reply_text' => esc_html__('Reply', 'ameron')
                                    ) ) );
                                    ?>
                                </div>
        ...
        public function comment_form_args($args = []){
            ...
            $pxl_comment_fields = array(
                'id_form'              => 'commentform',
                'title_reply'          => esc_attr__( 'Leave A Comment', 'ameron'),
                'title_reply_to'       => esc_attr__( 'Leave A Comment To ', 'ameron') . '%s',
                'cancel_reply_link'    => esc_attr__( 'Cancel Reply', 'ameron'),
                'title_reply_before'   => '<h4 id="reply-title" class="comment-reply-title">',
                'title_reply_after'    => '</h4>',
                'id_submit'            => 'submit',
                'class_submit'         => 'btn-primary',
                'label_submit'         =>  esc_attr__( 'Send Message', 'ameron'),
                'submit_button'        => '<button name="%1$s" type="submit" id="%2$s" class="%3$s"><span>%4$s</span></button>',
                'comment_notes_before' => '<label>'.esc_html__( 'Your email address will not be published. Required fields are marked *', 'ameron' ).'</label>',
                'comment_field'        =>  '',
            );
            ...
            $pxl_fields['author'] = '<div class="comment-form-field comment-form-author col-lg-4 col-md-4 col-sm-12">'.
                ...
                '" size="30" required placeholder="'.esc_attr__('Your Name*', 'ameron').'"/></div>';

            // email 
            $pxl_fields['email'] = '<div class="comment-form-field comment-form-email col-lg-4 col-md-4 col-sm-12">'.
                '<input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) .
                '" size="30" required placeholder="'.esc_attr__('Your Email*', 'ameron').'"/></div>';

            //phone
            if($this->show_phone == '1'){
                $pxl_fields['phone'] = '<div class="comment-form-field comment-form-phone col-lg-4 col-md-4 col-sm-12">'.
                '<input id="phone" name="phone" type="text" size="30" placeholder="'.esc_attr__('Phone', 'ameron').'"/></div>';
            }
            ...
            if($this->show_subject == '1'){
                ...
                '" size="30" placeholder="'.esc_attr__('Subject', 'ameron').'"/></div>';
            } 
            ...
            if ( has_action( 'set_comment_cookies', 'wp_set_comment_cookies' ) && get_option( 'show_comments_cookies_opt_in' ) ) {
                ...
                $pxl_fields['cookies'] = sprintf(
                    '<p class="comment-form-cookies-consent d-none">%s %s</p>',
                    sprintf(
                    ...
                    sprintf(
                        ...
                        esc_html__( 'Save my details in this browser for the next time I comment.', 'ameron' )
                    )
                );
            }
            ...
            $pxl_comment_fields['comment_field'] .= '<div class="pxl-comment-form-fields-wrap pxl-comment-form-fields-message row"><div class="comment-form-field comment-form-comment col-12"><textarea id="comment-msg" name="comment" cols="45" rows="8" required placeholder="'.esc_attr__('Message', 'ameron').'" aria-required="true">' .'</textarea></div></div>';
        ...
        function comment_product_form_args($args = []){
            ...
            $pxl_comment_fields = array(
                'id_form'              => 'commentform',
                'title_reply'          => esc_attr__( 'Leave a Review', 'ameron'),
                'title_reply_to'       => esc_attr__( 'Leave a Review To ', 'ameron') . '%s',
                'cancel_reply_link'    => esc_attr__( 'Cancel Reply', 'ameron'),
                'title_reply_before'   => '<h4 id="reply-title" class="comment-reply-title">',
                'title_reply_after'    => '</h4>',
                'id_submit'            => 'submit',
            ...
            $pxl_fields['author'] = '<div class="comment-form-field comment-form-author col-lg-4 col-md-4 col-sm-12">'.
                '<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) .
                '" size="30" placeholder="'.esc_attr__('Name*', 'ameron').'"/></div>';

            // email
            $pxl_fields['email'] = '<div class="comment-form-field comment-form-email col-lg-4 col-md-4 col-sm-12">'.
                '<input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) .
                '" size="30" placeholder="'.esc_attr__('Email*', 'ameron').'"/></div>';

            $pxl_fields['phone'] = '<div class="comment-form-field comment-form-phone col-lg-4 col-md-4 col-sm-12">'.
                '<input id="phone" name="phone" type="text" size="30" placeholder="'.esc_attr__('Mobile*', 'ameron').'"/></div>';
            ...
            $pxl_comment_fields['comment_field'] .= '<div class="pxl-comment-form-fields-wrap pxl-comment-form-fields-message row"><div class="comment-form-field comment-form-comment col-12"><textarea id="comment-msg" name="comment" cols="45" rows="8" placeholder="'.esc_attr__('Your review', 'ameron').'" aria-required="true">' .'</textarea></div></div>';
