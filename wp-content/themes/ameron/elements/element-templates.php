...
if(!function_exists('ameron_get_post_grid')){
    function ameron_get_post_grid($posts = [], $settings = []){
        ...
        switch ($layout) {
            case 'post-list-1':
                ameron_get_post_list_layout1($posts, $settings, $args_m);
                break;
            case 'post-list-2':
                ameron_get_post_list_layout2($posts, $settings, $args_m);
                break;
            case 'post-1':
                ameron_get_post_grid_layout1($posts, $settings, $args_m);
                break;
            case 'post-2':
                ameron_get_post_grid_layout2($posts, $settings, $args_m);
                break;
            case 'pxl-case-study-list-1':
                ameron_get_pxl_case_study_list_layout1($posts, $settings, $args_m);
                break;
            case 'pxl-case-study-1':
                ameron_get_post_grid_pxl_case_study1($posts, $settings, $args_m);
                break;
            case 'pxl-case-study-2':
                ameron_get_post_grid_pxl_case_study2($posts, $settings, $args_m);
                break;
            case 'pxl-practice-area-1':
                ameron_get_post_grid_pxl_practice_area1($posts, $settings, $args_m);
                break;
            case 'pxl-practice-area-2':
                ameron_get_post_grid_pxl_practice_area2($posts, $settings, $args_m);
                break;
            default:
                return false;
                break;
...
add_action( 'wp_ajax_ameron_get_pagination_html', 'ameron_get_pagination_html' );
add_action( 'wp_ajax_nopriv_ameron_get_pagination_html', 'ameron_get_pagination_html' );
function ameron_get_pagination_html(){
    try{
        if(!isset($_POST['query_vars'])){
            throw new Exception(__('Something went wrong while requesting. Please try again!', 'ameron'));
        }
        $query = new WP_Query($_POST['query_vars']);
        ob_start();
        ameron()->page->get_pagination( $query,  true );
        $html = ob_get_clean();
        wp_send_json(
            array(
                'status' => true,
                'message' => esc_attr__('Load Successfully!', 'ameron'),
                'data' => array(
                    'html' => $html,
...
add_action( 'wp_ajax_ameron_load_more_product_grid', 'ameron_load_more_product_grid' );
add_action( 'wp_ajax_nopriv_ameron_load_more_product_grid', 'ameron_load_more_product_grid' );
function ameron_load_more_product_grid(){
    try{
        ...
        $loop = ameron_woocommerce_query($query_type,$post_per_page,$product_ids,$categories,$param_args);
        extract($loop);
        ...
        if($posts->have_posts()){
            ...
            wp_send_json(
                array(
                    'status' => true,
                    'message' => esc_html__('Load Post Grid Successfully!', 'ameron'),
                    'data' => array(
                        'html'  => $html,
            ...
            wp_send_json(
                array(
                    'status' => false,
                    'message' => esc_html__('Load Post Grid No More!', 'ameron')
                )
            );
        } 
    }
...
add_action( 'wp_ajax_ameron_load_more_post_grid', 'ameron_load_more_post_grid' );
add_action( 'wp_ajax_nopriv_ameron_load_more_post_grid', 'ameron_load_more_post_grid' );
function ameron_load_more_post_grid(){
    try{
        if(!isset($_POST['settings'])){
            throw new Exception(__('Something went wrong while requesting. Please try again!', 'ameron'));
        }
        ...
        ameron_get_post_grid($posts, $settings);
        $html = ob_get_clean();
        wp_send_json(
            array(
                'status' => true,
...
function ameron_get_post_list_layout1($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
        ...
        $button_text = !empty($button_text) ? $button_text : esc_html__('Read more', 'ameron');
        $increase = $key + 1;
        $data_settings = '';
        $animate_cls = '';
                                            ...
                                            <div class="meta-inner d-flex align-items-center hover-underline">
                                                <?php if($show_author == 'true') : ?>
                                                    <span class="post-author col-auto d-flex">
                                                        <span><?php echo esc_html__('By', 'ameron')."&nbsp;"; ?> </span>
                                                ...
                                                <?php if($show_comment == 'true') : ?>
                                                    <span class="post-comments d-flex">
                                                        <a href="<?php echo get_comments_link($post->ID); ?>">
                                                            <span><?php comments_number(esc_html__('No Comments', 'ameron'), esc_html__(' 1', 'ameron'), esc_html__(' %', 'ameron'), $post->ID); ?></span>
                                                        </a>
                                                    </span>
                                                <?php endif; ?>
                                            </div>
...
function ameron_get_post_list_layout2($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
                                    ...
                                    <div class="meta-inner">
                                        <?php if($show_author == 'true') : ?>
                                            <div class="post-author d-flex align-items-center">
                                                <span><?php echo esc_html('POST:', 'ameron');?></span>
                                                <span>
                                                    <?php echo esc_html__('By', 'ameron'); ?>&nbsp;<?php the_author_posts_link(); ?>
                                                </span>
                                                <span class="author-avatar">
                                                    <?php echo get_avatar( get_the_author_meta( 'ID', $post->post_author ), 'thumbnail' ); ?>
                                                </span>
                                        ...
                                        <?php if($show_comment == 'true') : ?>
                                            <div class="post-comments d-flex align-items-center">
                                                <span><?php echo esc_html('COMMENTS:', 'ameron');?></span>
                                                <a href="<?php echo get_comments_link($post->ID); ?>">
                                                    <span><?php comments_number(esc_html__('No Comments', 'ameron'), esc_html__(' 1', 'ameron'), esc_html__(' % Comments', 'ameron'), $post->ID); ?></span>
                                                </a>
                                            </div>
                                        <?php endif; ?>
                                        <?php if($show_category == 'true') : ?>
                                            <div class="post-categories d-flex align-items-center hover-underline">
                                                <span><?php echo esc_html('CATEGORIES:', 'ameron');?></span>
                                                <span class="post-category d-flex"><span><?php the_terms( $post->ID, 'category', '', ', ', '' ); ?></span></span>
                                            </div>
                                        <?php endif; ?>
                                    </div>
...
function ameron_get_post_grid_layout1($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
        ...
        $show_category = ameron()->get_theme_opt( 'show_category', true );
            ...
            <div class="grid-item-inner">
                ...
                <div class="content">
                    <div class="item-content">
                        <?php if($show_date == 'true' || $show_author == 'true' || $show_category = 'true') : ?>
                            <div class="item-post-meta">
                                <div class="background-overlay"></div>
                                <div class="meta-inner d-flex align-items-center">
                                    <?php if( $show_author == 'true' ) : ?>
                                        <span class="post-author meta-item"><span><?php echo esc_html__('By', 'ameron'); ?></span>&nbsp;<a href="<?php echo esc_url(get_author_posts_url($post->post_author, $author->user_nicename)); ?>"><?php echo esc_html($author->display_name); ?></a></span>
                                    <?php endif; ?>
...
function ameron_get_post_grid_layout2($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
                            ...
                            <div class="item-post-meta">
                                <div class="meta-inner d-flex align-items-center">
                                    <?php if( $show_date == 'true' ) : ?>
                                        ...
                                        <?php echo get_the_date($date_formart, $post->ID); ?>
                                        </span>
                                    <?php endif; ?>
                                </div>
                            </div>
...
function ameron_get_pxl_case_study_list_layout1($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
...
function ameron_get_post_grid_pxl_case_study1($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
...
function ameron_get_post_grid_pxl_case_study2($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
        ...
        <div class="<?php echo esc_attr($str_item_class. ' ' .$animate_cls. ' '.$filter_class ); ?>" <?php pxl_print_html($data_settings); ?>>
            <div class="grid-item-inner cross-hover">
                <?php if (isset( $thumbnail )): ?>
                    <div class="item-featured">
                        <a href="<?php echo esc_url(get_permalink( $post->ID )); ?>"><?php pxl_print_html($thumbnail); ?></a>
                    </div>
                <?php endif; ?>
                <div class="item-content">
                    <div class="content-inner">
...
function ameron_get_post_grid_pxl_practice_area1($posts = [], $settings = [], $args_m = []){
    extract($settings);
    foreach ($posts as $key => $post):
        $str_item_class = !empty($args_m[$key]['item_class']) ? $args_m[$key]['item_class'] : $item_class;
        $filter_class = '';
        ...
        ?>
        <div class="<?php echo esc_attr($str_item_class. ' ' .$animate_cls. ' '.$filter_class ); ?>" <?php pxl_print_html($data_settings); ?>>
            <div class="grid-item-inner cross-hover-2">
                <div class="background-overlay-service"></div>
                <div class="item-content">
                    <div class="content-inner">
                        <?php
                        if ( $area_icon == true){
